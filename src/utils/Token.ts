 ...
 * @Last Modified time: 2021-04-25 16:56:13
 * Token 相关处理
 */
...
export class TokenService {
    ...
    async refreshToken(user: IUserOptions, token: string){
        try{
            ...
            if(decoded) {
                ...
                if(decoded.expire + decoded.time < new Date().getTime()) {
                    this.ctx.body = {
                        code : '-100',
                        message: 'token过期'
                    }
     ...
     * @returns 
     */
    async verifyToken(user: IUserOptions, token: string):Promise<boolean> {
        const cacheToken = await this.cache.get(this.constants.PC_ADMIN + user.username);
        if(cacheToken === token){
            const decoded = jwt.verify(token, this.config.secret);
            if(decoded.expire + decoded.time < new Date().getTime()) {
                this.ctx.body = {
